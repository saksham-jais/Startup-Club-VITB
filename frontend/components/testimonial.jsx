...
export default function App() {
  ...
  useEffect(() => {
    if (isDragging || padding === 0) return;
  ...
  useEffect(() => {
    ...
    if (currentRef) currentRef.addEventListener('scroll', handleScrollUpdate);

    return () => {
      if (currentRef) currentRef.removeEventListener('scroll', handleScrollUpdate);
    };
  }, [padding]);
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-white text-white font-inter p-4">
        ...
        >
          {CARD_DATA.map((card, index) => (
            <div
              ...
              className={`group flex-shrink-0 w-80 bg-blue-400 rounded-xl shadow-xl border-4 border-white/5 transition-all duration-500 ease-out snap-center relative ${
